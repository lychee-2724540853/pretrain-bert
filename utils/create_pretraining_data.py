...
def read_fault(filepath=None, seps='。'):
    import os
    def read1file(filepath, seps='。'):
        with open(filepath, 'r', encoding='utf8') as f:
            lines = f.readlines()
        paragraphs = []
        for line in lines:
    ...
    if os.path.isfile(filepath):
        return read1file(filepath, seps)
    else:
        results = []
        filelist = os.listdir(filepath)
        for file in filelist:
            results.extend(read1file(os.path.join(filepath, file), seps))
        return results
...
class LoadBertPretrainingDataset(object):
    ...
    def get_format_data(self, filepath):
        ...
        elif self.data_name == 'fault':
            return read_fault(filepath, self.seps)
        else:
